...
#   for_each   = module.this.enabled ? local.task_policy_arns_map : {}
...
#   for_each = module.this.enabled ? toset(["true"]) : toset([])
#   name     = module.exec_label.id
...
#   for_each   = module.this.enabled ? local.task_exec_policy_arns_map : {}
#   policy_arn = each.value
#   role       = one(aws_iam_role.ecs_exec[*]["id"])
# }
...
#   # permissions_boundary = var.permissions_boundary == "" ? null : var.permissions_boundary
#   tags                 = module.service_connect_label.tags
# }
